...
TRACE_SESSION_ARG=${RT_TRACE_SESSION_ID:-"-"}
TRACE_MPG_ARG=${RT_TRACE_MEASUREMENTS_PER_GROUP:-"-"}

"$SCRIPT_DIR/stop_supervised_stack.sh" "$VISIONFIVE" "$ROCKPI"

ssh "$VISIONFIVE" sh -s -- "$SUPERVISOR_BIN" "$RUNTIME_WRAPPER" \
	"$INTERFACE" "$TIMEOUT_US" "$TRACE_SESSION_ARG" \
...
ssh "$VISIONFIVE" sh -s -- "$ROCKPI" "$CONTROLLER_BIN" "$INTERFACE" \
	"$TRACE_SESSION_ARG" "$TRACE_MPG_ARG" \
	"$RT_TRACE_CONTROLLER_PATH" "$RT_TRACE_EXPORTERS" <<'VISIONFIVE_TO_ROCKPI'
set -eu
ROCKPI=$1
CONTROLLER_BIN=$2
INTERFACE=$3
...
ssh "$ROCKPI" sh -s -- "$CONTROLLER_BIN" "$INTERFACE" \
	"$TRACE_SESSION_ID" "$TRACE_MPG" "$TRACE_PATH" \
	"$TRACE_EXPORTERS" <<'ROCKPI_REMOTE'
set -eu
CONTROLLER_BIN=$1
INTERFACE=$2
TRACE_SESSION_ID=$3
